...
-   Note from SC 2.1.2:
    -   Note 1: Since any content that does not meet this success criterion can interfere with a user's ability to use the whole {% if page.source == "Web" %}page{% elsif page.source == "Doc" %}document{% endif %}, all content {% if page.source == "Web" %}on the Web page{% elsif page.source == "Doc" %}in the document{% endif %} (whether it is used to meet other success criteria or not) must meet this success criterion. See Conformance Requirement 5: Non-Interference.
